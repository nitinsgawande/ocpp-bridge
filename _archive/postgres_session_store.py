...
# These are async functions — call with await from async context (FastAPI).
...
async def write_audit(charger_id: str, transaction_id: int, **kwargs) -> None:
    """
    Upsert a row in session_audit for every state transition.
    ...
    try:
        ...
        row_id = f"{charger_id}:{transaction_id}"

        async with AsyncSession(engine) as db:
            result = await db.exec(
                select(SessionAudit).where(SessionAudit.id == row_id)
            )
            row = result.first()
            ...
            if row:
                ...
                row = SessionAudit(
                    ...
                    **{k: v for k, v in kwargs.items()
                       if k in SessionAudit.__fields__}
                )
                db.add(row)
            await db.commit()
    except Exception as e:
        log.error(f"[{charger_id}] Postgres write_audit failed "
...
async def log_meter(charger_id: str, transaction_id: int,
    ...
    """
    Append one row to meter_log for every MeterValues reading.
    This is the tamper-evident audit trail for billing disputes.
    """
    try:
        from sqlmodel.ext.asyncio.session import AsyncSession
        from sqlalchemy.ext.asyncio import create_async_engine
        from db import MeterLog
        ...
        log.error(f"[{charger_id}] Postgres log_meter failed "
                  f"txn={transaction_id}: {e}")
